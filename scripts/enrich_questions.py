...
try:
    ...
    if not firebase_admin._apps:
        # Aponta para a chave de serviço que já usamos no app.py
...
def enrich_and_save_question(raw_question):
    """
    Pega uma questão 'crua', usa a IA para enriquecê-la e salva no Firestore.
    """
    ...
    4.  **Gere uma Explicação Completa:** Crie uma explicação clara e didática. Se for múltipla escolha, explique por que a correta está certa e as outras erradas. Se for Certo/Errado, fundamente a resposta.
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.2 # Usamos baixa temperatura para tarefas de classificação
        )
    ...
    # 3. Combina os dados originais com os dados enriquecidos
    final_question = {
        ...
        "status_revisao": "aprovado" # Já inserimos como 'aprovado'
    }

    # 4. Salva no Firestore
    try:
        # Usaremos o ID da coleção como um identificador único
        questions_collection = db.collection('banco_questoes')
        # O Firestore gerará um ID automático para o novo documento
